    ...
    <>
      ...
      <section class={`${mode} max-w-screen-sm mx-auto space-y-6`}>
        {isLoading() && Array.from(Array(limit)).map(() =>
          <article class="flex gap-2 px-10 py-4 bg-zinc-50 rounded-lg drop-shadow-sm border border-gray-200 animate-pulse">
        ...
        {!isLoading() && feedData().map(post => <article class="px-10 py-4 mx-auto bg-zinc-50 rounded-lg drop-shadow-sm border border-gray-200">
            ...
            <div>
              <div class="flex max-w-[calc(100vw-96px)] items-center">
                <a href={`https://bsky.app/profile/${post.handle}`} class="text-ellipsis overflow-hidden whitespace-nowrap hover:underline dark:text-white">
                  <span class="font-bold mr-2 text-gray-700 dark:text-white">{post.username}</span>
              ...
              <p class="text-base/snug whitespace-pre-line dark:text-white">
